package org.com2031.labs.greedy.css1ss;
...
//import IntervalScheduling.Interval;
 ...
 */
	...
	static boolean overlaps(Interval i, Interval j) {
		boolean crash;
		if(i.start >= j.finish || j.start >= i.finish) {
			crash = false;
		}else {crash = true;}
		return crash;
	...
	static List<List<Interval>> partition(Interval[] intervals) {
		...
		Arrays.sort(intervals, new Comparator<Interval>() {
			...
			public int compare(Interval first, Interval second) {
				return first.start - second.start;  //TODO: replace this with the correct code for compare;
			}
		});
